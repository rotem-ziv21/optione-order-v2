...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting handle_order_paid function. NEW.status=' || NEW.status || ', OLD.status=' || OLD.status);
  
  -- בדיקה אם השדה status השתנה ל-'paid'
  IF (NEW.status = 'paid' AND OLD.status != 'paid') THEN
    -- לוג שהתנאי התקיים
    INSERT INTO debug_logs (message) VALUES ('Status changed to paid. Order ID=' || NEW.id);
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished handle_order_paid function');
  RETURN NEW;
END;
