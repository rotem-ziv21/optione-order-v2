...
-- פונקציה שתופעל כאשר הזמנה בטבלת customer_orders מסומנת כ-completed
CREATE OR REPLACE FUNCTION public.handle_customer_orders_completed()
RETURNS TRIGGER AS $$
DECLARE
  webhook_url TEXT;
  webhook_id UUID;
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting handle_customer_orders_completed function. NEW.status=' || NEW.status || ', OLD.status=' || OLD.status);
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished handle_customer_orders_completed function');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- טריגר שיפעיל את הפונקציה כאשר הזמנה בטבלת customer_orders מתעדכנת
DROP TRIGGER IF EXISTS trigger_customer_orders_completed ON customer_orders;
CREATE TRIGGER trigger_customer_orders_completed
AFTER UPDATE ON customer_orders
FOR EACH ROW
EXECUTE FUNCTION handle_customer_orders_completed();
...
FROM pg_proc
WHERE proname = 'handle_customer_orders_completed';
