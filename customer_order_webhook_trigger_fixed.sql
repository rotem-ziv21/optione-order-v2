...
INSERT INTO debug_logs (message) VALUES ('Starting webhook trigger setup');

-- פונקציה שתופעל כאשר הזמנה בטבלת customer_order מסומנת כ-completed
CREATE OR REPLACE FUNCTION public.handle_customer_order_completed()
RETURNS TRIGGER AS $$
DECLARE
  webhook_url TEXT;
  webhook_id UUID;
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting handle_customer_order_completed function. NEW.status=' || NEW.status || ', OLD.status=' || OLD.status);
  
  -- בדיקה אם השדה status השתנה ל-'completed'
  IF (NEW.status = 'completed' AND OLD.status != 'completed') THEN
    -- לוג שהתנאי התקיים
    INSERT INTO debug_logs (message) VALUES ('Status changed to completed. Order ID=' || NEW.id);
    ...
    END LOOP;
  ELSE
    INSERT INTO debug_logs (message) VALUES ('Status condition not met. NEW.status=' || NEW.status || ', OLD.status=' || OLD.status);
  END IF;
  
  INSERT INTO debug_logs (message) VALUES ('Finished handle_customer_order_completed function');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- טריגר שיפעיל את הפונקציה כאשר הזמנה בטבלת customer_order מתעדכנת
DROP TRIGGER IF EXISTS trigger_customer_order_completed ON customer_order;
CREATE TRIGGER trigger_customer_order_completed
AFTER UPDATE ON customer_order
FOR EACH ROW
EXECUTE FUNCTION handle_customer_order_completed();

-- לוג שהטריגר נוצר בהצלחה
INSERT INTO debug_logs (message) VALUES ('Trigger created successfully');

-- בדיקת הטריגר
SELECT tgname, tgrelid::regclass, tgenabled
FROM pg_trigger
WHERE tgname = 'trigger_customer_order_completed';

-- בדיקת הפונקציה
SELECT proname, proargtypes, prosrc
FROM pg_proc
WHERE proname = 'handle_customer_order_completed';
