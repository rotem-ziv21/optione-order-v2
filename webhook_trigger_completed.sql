...
-- פונקציה שתופעל כאשר הזמנה מסומנת כהושלמה
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting handle_order_completed function. NEW.status=' || NEW.status || ', OLD.status=' || OLD.status);
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished handle_order_completed function');
  RETURN NEW;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;

-- טריגר שיפעיל את הפונקציה כאשר הזמנה מתעדכנת
DROP TRIGGER IF EXISTS trigger_order_completed ON orders;
