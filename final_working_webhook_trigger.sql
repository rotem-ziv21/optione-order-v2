-- טריגר סופי שישלח webhook לכתובת קיימת

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting final working webhook trigger setup');
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting final webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    -- שליפת פרטי הלקוח - בדיקה אילו עמודות קיימות בטבלה
    BEGIN
      INSERT INTO debug_logs (message) VALUES ('Trying to find customer');
      
      -- בדיקה אם העמודה customer_id קיימת
      BEGIN
        EXECUTE 'SELECT EXISTS (
          SELECT 1 FROM information_schema.columns 
          WHERE table_name = ''customers'' AND column_name = ''customer_id''
        )' INTO STRICT webhook_endpoint;
        
        IF webhook_endpoint = 'true' THEN
        ...
        ELSE
          INSERT INTO debug_logs (message) VALUES ('customer_id column does not exist, using id field');
    ...
    -- בדיקה אם המוצר הוא עגיל זהב ושינוי ה-payload בהתאם
    IF product_name = 'עגיל זהב' OR product_name LIKE '%זהב%' THEN
      -- הוספת שדה product_type ל-payload
      payload := payload || jsonb_build_object(
        'product_type', 'jewelry',
        'special_handling', true
      );
      INSERT INTO debug_logs (message) VALUES ('Added jewelry-specific fields to payload');
    END IF;
    ...
    LOOP
      ...
      -- שליחת ה-webhook - תמיד לכתובת הכללית שקיימת
      BEGIN
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished final webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Final working webhook trigger created successfully');
