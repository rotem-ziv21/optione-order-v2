-- פתרון שישתמש ב-webhook.site לבדיקת שליחת webhook למוצרי זהב

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting webhook.site solution setup');

-- פונקציה שתשלח webhook למוצרי זהב ל-webhook.site ולשאר המוצרים לכתובת הרגילה
...
DECLARE
  ...
  response_status INT;
  response_body TEXT;
  gold_webhook_sent BOOLEAN := FALSE;
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting webhook.site function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    BEGIN
      ...
      IF customer_record IS NULL THEN
        INSERT INTO debug_logs (message) VALUES ('Customer not found');
      ELSE
    ...
    -- בדיקה אם המוצר הוא עגיל זהב או מכיל את המילה "זהב"
    IF product_name = 'עגיל זהב' OR product_name LIKE '%זהב%' THEN
      INSERT INTO debug_logs (message) VALUES ('Gold jewelry product detected. Sending to webhook.site');
      
      -- צור כתובת webhook.site חדשה בכתובת https://webhook.site/
      -- והחלף את הכתובת הבאה בכתובת שקיבלת
      webhook_url := 'https://webhook.site/a1b2c3d4-e5f6-g7h8-i9j0-k1l2m3n4o5p6';
      webhook_id := 'd9b5225c-0eaf-43ed-9789-f3c54cad20c8';  -- השתמש במזהה הקיים
      
      INSERT INTO debug_logs (message) VALUES ('Sending gold jewelry webhook to: ' || webhook_url);
      ...
      BEGIN
        ...
        INSERT INTO debug_logs (message) VALUES ('Gold jewelry webhook response: status=' || response_status || ', body=' || response_body);
        gold_webhook_sent := TRUE;
      EXCEPTION WHEN OTHERS THEN
        INSERT INTO debug_logs (message) VALUES ('Error in http_post to webhook.site: ' || SQLERRM);
      ...
      BEGIN
        ...
        INSERT INTO debug_logs (message) VALUES ('Gold jewelry webhook log saved');
      EXCEPTION WHEN OTHERS THEN
        INSERT INTO debug_logs (message) VALUES ('Error saving gold jewelry webhook log: ' || SQLERRM);
      END;
    END IF;
    
    -- שליפת כל ה-webhooks המוגדרים לתשלום הזמנה
    -- אם זה מוצר זהב, נשלח גם לכתובת הרגילה וגם לwebhook.site
    FOR webhook_url, webhook_id IN 
      SELECT bw.url, bw.id 
      FROM business_webhooks bw
      WHERE bw.business_id = NEW.business_id AND bw.on_order_paid = TRUE
    LOOP
      INSERT INTO debug_logs (message) VALUES ('Found regular webhook: URL=' || webhook_url || ', ID=' || webhook_id);
      
      -- שליחת ה-webhook
      BEGIN
        INSERT INTO debug_logs (message) VALUES ('Sending regular webhook to: ' || webhook_url);
        ...
        BEGIN
          ...
          INSERT INTO debug_logs (message) VALUES ('Regular webhook response: status=' || response_status || ', body=' || response_body);
        EXCEPTION WHEN OTHERS THEN
        ...
        BEGIN
          ...
          INSERT INTO debug_logs (message) VALUES ('Regular webhook log saved');
        EXCEPTION WHEN OTHERS THEN
          INSERT INTO debug_logs (message) VALUES ('Error saving regular webhook log: ' || SQLERRM);
        END;
      EXCEPTION WHEN OTHERS THEN
    ...
    -- בדיקה אם נשלח webhook לתכשיטי זהב
    IF product_name = 'עגיל זהב' OR product_name LIKE '%זהב%' THEN
      IF gold_webhook_sent THEN
        INSERT INTO debug_logs (message) VALUES ('Successfully sent webhook for gold jewelry product');
      ELSE
        INSERT INTO debug_logs (message) VALUES ('Failed to send webhook for gold jewelry product');
      END IF;
    END IF;
  ELSE
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished webhook.site function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Webhook.site solution created successfully');
