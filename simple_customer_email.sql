-- פתרון פשוט למשיכת אימייל מטבלת customers

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting simple customer email webhook solution setup');
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting simple customer email webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    -- שליפת אימייל ישירות מטבלת customers
    BEGIN
      -- שליפת האימייל לפי customer_id
      ...
      ELSE
        INSERT INTO debug_logs (message) VALUES ('Email not found in customers table');
        customer_email := 'unknown@optione.co.il';  -- אימייל ברירת מחדל
      END IF;
    EXCEPTION WHEN OTHERS THEN
      INSERT INTO debug_logs (message) VALUES ('Error fetching email from customers: ' || SQLERRM);
      customer_email := 'unknown@optione.co.il';  -- אימייל ברירת מחדל במקרה של שגיאה
    END;
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished simple customer email webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Simple customer email webhook solution created successfully');
