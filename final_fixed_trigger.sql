...
INSERT INTO debug_logs (message) VALUES ('Starting final fixed trigger setup');
...
DECLARE
  webhook_url TEXT;
  webhook_id UUID;
  business_id_var UUID;  -- שינוי שם המשתנה כדי למנוע התנגשות
  product_id UUID;
  customer_record RECORD;
  product_record RECORD;
  order_item_record RECORD;
  payload JSONB;
  payload_text TEXT;  -- משתנה חדש לשמירת ה-payload כטקסט
  response_status INT;
  response_body TEXT;
BEGIN
  -- לוג התחלת הפונקציה
  ...
  IF (NEW.status = 'completed') THEN
    ...
    LOOP
      ...
      -- שליפת פרטי הלקוח
      BEGIN
        -- תיקון: המרה מפורשת של customer_id ל-UUID
      ...
      payload := jsonb_build_object(
        'event', 'order_paid',
        'order_id', NEW.id,
        'business_id', business_id_var,  -- שימוש בשם המשתנה החדש
        'timestamp', now()
      );
      ...
      -- המרת ה-payload ל-TEXT
      payload_text := payload::text;
      
      INSERT INTO debug_logs (message) VALUES ('Payload created: ' || payload_text);
      ...
      BEGIN
        ...
        BEGIN
          -- תיקון: שימוש ב-payload_text במקום payload::text
          SELECT
            status,
            content
          INTO
...
INSERT INTO debug_logs (message) VALUES ('Final fixed trigger created successfully');
