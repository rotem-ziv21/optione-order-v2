-- פתרון סופי עם אימייל קבוע ללקוח ספציפי

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting fixed webhook email solution setup');

-- פונקציה לשליחת webhook עם אימייל קבוע של לקוח
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting fixed webhook email function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    -- קביעת אימייל ספציפי ללקוח
    IF NEW.customer_id = 'Xz2TM4XrHWm1FTW7q0ru' THEN
      customer_email := 'actual-customer-email@gmail.com';  -- שים כאן את האימייל האמיתי של הלקוח
      INSERT INTO debug_logs (message) VALUES ('Using fixed email for customer ID Xz2TM4XrHWm1FTW7q0ru: ' || customer_email);
    ELSE
      -- ניסיון לשלוף את האימייל מטבלת customers
      BEGIN
        ...
        IF customer_email IS NOT NULL THEN
          INSERT INTO debug_logs (message) VALUES ('Found email in customers table: ' || customer_email);
        ELSE
        ...
        END IF;
      EXCEPTION WHEN OTHERS THEN
        customer_email := 'default@optione.co.il';  -- אימייל ברירת מחדל במקרה של שגיאה
        INSERT INTO debug_logs (message) VALUES ('Error fetching from customers, using default email: ' || customer_email);
      END;
    END IF;
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished fixed webhook email function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Fixed webhook email solution created successfully');
