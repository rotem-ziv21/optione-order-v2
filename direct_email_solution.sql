-- פתרון פשוט וישיר לשליחת אימייל של לקוח ב-webhook

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting direct email webhook solution setup');

-- פונקציה פשוטה לשליחת webhook עם אימייל של לקוח
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting direct email webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    -- שליפת אימייל הלקוח ישירות מטבלת auth.users
    BEGIN
      INSERT INTO debug_logs (message) VALUES ('Trying to find email directly from auth.users for customer_id=' || NEW.customer_id);
      ...
      BEGIN
        ...
        ELSE
          INSERT INTO debug_logs (message) VALUES ('Email not found in auth.users');
          ...
          BEGIN
            ...
            ELSE
              INSERT INTO debug_logs (message) VALUES ('Email not found in customers');
              ...
              BEGIN
                ...
                ELSE
                  INSERT INTO debug_logs (message) VALUES ('Email not found in users');
                  ...
                  BEGIN
                    ...
                    ELSE
                      INSERT INTO debug_logs (message) VALUES ('Email not found in profiles');
                    END IF;
                  EXCEPTION WHEN OTHERS THEN
      ...
      END;
    EXCEPTION WHEN OTHERS THEN
      INSERT INTO debug_logs (message) VALUES ('Error in email lookup: ' || SQLERRM);
    END;
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished direct email webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Direct email webhook solution created successfully');
