-- פתרון שישלח webhook לפי מזהה מוצר ספציפי

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting product-specific webhook solution setup');

-- פונקציה שתשלח webhook לפי מזהה מוצר ספציפי
    ...
    IF product_id IS NOT NULL THEN
      FOR webhook_url, webhook_id IN 
        ...
        WHERE bw.business_id = NEW.business_id 
          ...
          AND bw.product_id = product_id
      LOOP
        product_specific_webhook_found := TRUE;
        INSERT INTO debug_logs (message) VALUES ('Found product-specific webhook: URL=' || webhook_url || ', ID=' || webhook_id || ', Product ID=' || product_id);
...
INSERT INTO debug_logs (message) VALUES ('Product-specific webhook solution created successfully');
