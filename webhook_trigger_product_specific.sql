-- טריגר משופר שישלח webhook לכתובת המתאימה לפי סוג המוצר

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting product-specific webhook trigger setup');

-- פונקציה משופרת שתבחר את ה-webhook המתאים לפי סוג המוצר
...
DECLARE
  ...
  response_status INT;
  response_body TEXT;
  webhook_found BOOLEAN := FALSE;
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting product-specific webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
      ...
      ELSE
        ...
        BEGIN
          ...
          ELSE
            ...
            -- שמירת קטגוריית המוצר אם קיימת
            product_category := COALESCE(product_record.category, 'default');
            INSERT INTO debug_logs (message) VALUES ('Product category: ' || product_category);
          END IF;
    ...
    -- חיפוש webhook ספציפי לקטגוריית המוצר
    FOR webhook_url, webhook_id IN 
      SELECT bw.url, bw.id 
      FROM business_webhooks bw
      WHERE bw.business_id = NEW.business_id 
        AND bw.on_order_paid = TRUE
        AND (bw.category = product_category OR bw.category IS NULL)
      ORDER BY 
        -- תן עדיפות לwebhook ספציפי לקטגוריה
        CASE WHEN bw.category = product_category THEN 0 ELSE 1 END
      LIMIT 1
    LOOP
      webhook_found := TRUE;
      INSERT INTO debug_logs (message) VALUES ('Found specific webhook: URL=' || webhook_url || ', ID=' || webhook_id || ', Category=' || COALESCE(product_category, 'NULL'));
    ...
    -- אם לא נמצא webhook ספציפי, שלח לכל ה-webhooks הכלליים
    IF NOT webhook_found THEN
      INSERT INTO debug_logs (message) VALUES ('No specific webhook found for category. Sending to all general webhooks.');
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished product-specific webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Product-specific webhook trigger created successfully');
