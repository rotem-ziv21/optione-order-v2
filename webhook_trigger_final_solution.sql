...
INSERT INTO debug_logs (message) VALUES ('Starting webhook final solution setup');
...
BEGIN
  ...
  IF (NEW.status = 'completed') THEN
    ...
    LOOP
      ...
      -- שליפת פרטי הלקוח - דילוג על שגיאות המרה
      BEGIN
        -- ניסיון לשלוף את הלקוח בלי המרה מפורשת
        BEGIN
        ...
        EXCEPTION WHEN OTHERS THEN
          -- ניסיון נוסף - אולי customer_id הוא טקסט ולא UUID
          BEGIN
            INSERT INTO debug_logs (message) VALUES ('Trying alternative customer lookup');
            -- בדיקה אם יש לקוח עם מזהה טקסט
            SELECT * INTO customer_record 
            FROM customers c 
            WHERE c.id::TEXT = NEW.customer_id::TEXT;
            
            IF customer_record IS NULL THEN
              INSERT INTO debug_logs (message) VALUES ('Customer still not found after text comparison');
            ELSE
              INSERT INTO debug_logs (message) VALUES ('Customer found with text comparison: ' || customer_record.name);
            END IF;
          EXCEPTION WHEN OTHERS THEN
            INSERT INTO debug_logs (message) VALUES ('Error in alternative customer lookup: ' || SQLERRM);
      ...
      BEGIN
        ...
        -- תיקון בעיית http_request.content is NULL
        BEGIN
          -- בדיקה שה-payload לא ריק
          IF payload::text IS NULL OR payload::text = '' OR payload::text = '{}' THEN
            INSERT INTO debug_logs (message) VALUES ('Error: Payload is empty or null');
          ELSE
            -- שימוש בפונקציית http_post במקום http
            BEGIN
              SELECT
                status,
                content
...
INSERT INTO debug_logs (message) VALUES ('Webhook final solution trigger created successfully');
