...
-- פונקציה רגילה שאפשר לקרוא לה ישירות
CREATE OR REPLACE FUNCTION public.send_order_webhooks(order_id UUID)
RETURNS VOID AS $$
DECLARE
  webhook_url TEXT;
  webhook_id UUID;
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting send_order_webhooks function for order_id=' || order_id);
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished send_order_webhooks function');
END;
