-- פתרון ישיר לשליחת webhook למוצרי זהב לכתובת ספציפית

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting direct webhook fix setup');

-- פונקציה פשוטה שתשלח webhook לכתובת הנכונה לפי שם המוצר
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting direct webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    -- בדיקה אם המוצר הוא עגיל זהב ושליחה לכתובת המתאימה
    IF product_name = 'עגיל זהב' OR product_name LIKE '%זהב%' THEN
      INSERT INTO debug_logs (message) VALUES ('Gold jewelry product detected. Sending to specific webhook.');
      
      -- שליחה ישירה לכתובת ספציפית לתכשיטי זהב
      BEGIN
        webhook_url := 'https://webhook.site/your-unique-id';  -- שנה לכתובת webhook.site שלך
        webhook_id := 'd9b5225c-0eaf-43ed-9789-f3c54cad20c8';  -- השתמש במזהה הקיים
        
        INSERT INTO debug_logs (message) VALUES ('Sending webhook to webhook.site: ' || webhook_url);
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished direct webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Direct webhook fix created successfully');
