-- פתרון משופר שישלח webhook עם פרטי לקוח מלאים - גרסה מתוקנת

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting fixed customer data webhook solution setup');
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting fixed customer data webhook function. NEW.status=' || NEW.status);
    ...
    IF customer_record IS NOT NULL THEN
      ...
      BEGIN
        ...
        LOOP
          ...
          IF col_record.column_name NOT IN ('created_at', 'updated_at') THEN
            BEGIN
              -- בדיקה אם הערך קיים ולא NULL
              EXECUTE format('SELECT ($1).%I IS NOT NULL', col_record.column_name)
              INTO STRICT webhook_url
              USING customer_record;
              
              IF webhook_url = 'true' THEN
                -- הוספת השדה ל-JSON
                EXECUTE format('SELECT jsonb_build_object(%L, ($1).%I::TEXT)', 
                               col_record.column_name, col_record.column_name)
                INTO STRICT webhook_url
                USING customer_record;
                
                customer_obj := customer_obj || webhook_url::JSONB;
              END IF;
            EXCEPTION WHEN OTHERS THEN
              INSERT INTO debug_logs (message) VALUES ('Error adding field ' || col_record.column_name || ' to customer object: ' || SQLERRM);
            END;
          END IF;
        END LOOP;
        ...
        INSERT INTO debug_logs (message) VALUES ('Added dynamic customer data to payload');
      EXCEPTION WHEN OTHERS THEN
        ...
        BEGIN
          customer_obj := jsonb_build_object('id', customer_record.id);
          
          -- הוספת שדות נפוצים אם הם קיימים
          BEGIN
            EXECUTE 'SELECT $1.name' INTO STRICT webhook_url USING customer_record;
            customer_obj := customer_obj || jsonb_build_object('name', webhook_url);
          EXCEPTION WHEN OTHERS THEN
            NULL;
          END;
          
          BEGIN
            EXECUTE 'SELECT $1.email' INTO STRICT webhook_url USING customer_record;
            customer_obj := customer_obj || jsonb_build_object('email', webhook_url);
          EXCEPTION WHEN OTHERS THEN
            NULL;
          END;
          
          BEGIN
            EXECUTE 'SELECT $1.phone' INTO STRICT webhook_url USING customer_record;
            customer_obj := customer_obj || jsonb_build_object('phone', webhook_url);
          EXCEPTION WHEN OTHERS THEN
            NULL;
          END;
          
          BEGIN
            EXECUTE 'SELECT $1.contact_id' INTO STRICT webhook_url USING customer_record;
            customer_obj := customer_obj || jsonb_build_object('contact_id', webhook_url);
          EXCEPTION WHEN OTHERS THEN
            NULL;
          END;
        ...
        EXCEPTION WHEN OTHERS THEN
          INSERT INTO debug_logs (message) VALUES ('Error building basic customer object: ' || SQLERRM);
          
          -- אם גם זה נכשל, הוסף רק את ה-id
          payload := payload || jsonb_build_object(
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished fixed customer data webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Fixed customer data webhook solution created successfully');
