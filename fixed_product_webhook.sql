-- פתרון מתוקן שישלח webhook לפי מזהה מוצר ספציפי

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting fixed product webhook solution setup');

-- פונקציה שתשלח webhook לפי מזהה מוצר ספציפי - עם תיקון להתנגשות שמות
...
DECLARE
  ...
  product_id_var UUID;  -- שינוי שם המשתנה למניעת התנגשות
  product_name TEXT;
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting fixed product webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    BEGIN
      ...
      ELSE
        ...
        product_id_var := order_item_record.product_id;  -- שימוש בשם המשתנה החדש
        
        -- שליפת פרטי המוצר
        BEGIN
          SELECT * INTO product_record FROM products p WHERE p.id = product_id_var;  -- שימוש בשם המשתנה החדש
          IF product_record IS NULL THEN
            INSERT INTO debug_logs (message) VALUES ('Product not found for ID=' || product_id_var);  -- שימוש בשם המשתנה החדש
          ELSE
    ...
    IF product_id_var IS NOT NULL THEN
      FOR webhook_url, webhook_id IN 
        ...
        WHERE bw.business_id = NEW.business_id 
          ...
          AND bw.product_id = product_id_var  -- שימוש בשם המשתנה החדש
      LOOP
        product_specific_webhook_found := TRUE;
        INSERT INTO debug_logs (message) VALUES ('Found product-specific webhook: URL=' || webhook_url || ', ID=' || webhook_id || ', Product ID=' || product_id_var);  -- שימוש בשם המשתנה החדש
        ...
        BEGIN
          ...
          BEGIN
            INSERT INTO webhook_logs (
              ...
              webhook_id,
              NEW.id,
              product_id_var,  -- שימוש בשם המשתנה החדש
              payload,
              response_status,
              response_body,
              now()
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished fixed product webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Fixed product webhook solution created successfully');
