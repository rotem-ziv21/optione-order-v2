-- פתרון עם אימייל קבוע ספציפי ללקוח

-- הוספת רשומת בדיקה לטבלת הלוגים
INSERT INTO debug_logs (message) VALUES ('Starting hardcoded email webhook solution setup');
...
BEGIN
  -- לוג התחלת הפונקציה
  INSERT INTO debug_logs (message) VALUES ('Starting hardcoded email webhook function. NEW.status=' || NEW.status);
  ...
  IF (NEW.status = 'completed') THEN
    ...
    -- קביעת אימייל ספציפי לפי מזהה הלקוח
    IF NEW.customer_id = 'Xz2TM4XrHWm1FTW7q0ru' THEN
      customer_email := 'rotem@optione.co.il';  -- אימייל ספציפי ללקוח זה
      ...
      BEGIN
        ...
        ELSE
          ...
          BEGIN
            ...
            IF customer_email IS NOT NULL THEN
              INSERT INTO debug_logs (message) VALUES ('Found email in customers: ' || customer_email);
            ELSE
              -- אימייל ברירת מחדל אם לא נמצא אימייל
              customer_email := 'unknown@optione.co.il';
              INSERT INTO debug_logs (message) VALUES ('Using default email for unknown customer: ' || customer_email);
            END IF;
          EXCEPTION WHEN OTHERS THEN
            INSERT INTO debug_logs (message) VALUES ('Error fetching from customers: ' || SQLERRM);
            customer_email := 'unknown@optione.co.il';
          END;
        END IF;
      EXCEPTION WHEN OTHERS THEN
  ...
  INSERT INTO debug_logs (message) VALUES ('Finished hardcoded email webhook function');
  RETURN NEW;
EXCEPTION WHEN OTHERS THEN
  -- לוג של שגיאה כללית, אבל לא נפסיק את התהליך
...
INSERT INTO debug_logs (message) VALUES ('Hardcoded email webhook solution created successfully');
